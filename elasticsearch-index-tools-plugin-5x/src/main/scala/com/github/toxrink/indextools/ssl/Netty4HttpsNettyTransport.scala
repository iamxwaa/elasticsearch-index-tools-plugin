...
class Netty4HttpsNettyTransport(settings: Settings,
  ...
  override def configureServerChannelHandler(): ChannelHandler = {
    new SSLHttpChannelHandler(this, this.detailedErrorsEnabled, this.threadPool.getThreadContext)
  }

  class SSLHttpChannelHandler(transport: Netty4HttpServerTransport,
