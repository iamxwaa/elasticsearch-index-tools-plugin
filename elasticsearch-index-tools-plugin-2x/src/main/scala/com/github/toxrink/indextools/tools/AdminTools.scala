...
object AdminTools {
  ...
  def checkAndCreateIndex(srcIndex: String, testAlias: String, client: Client): (Boolean, Boolean) = {
    val indexExist = new IndicesExistsRequest(srcIndex)
    val isExist =
      client.admin().indices().exists(indexExist).actionGet().isExists
    if (!isExist) {
      val create = new CreateIndexRequest(srcIndex)
  ...
  def writeRecord(srcIndex: String, status: Boolean, aliasIndex: List[(String, (Date, Date))], client: Client): Unit = {
    //记录索引不存在则创建
    if (!recordIndexExist) {
      val indexExist = new IndicesExistsRequest(IndexToolsConstants.ALIAS_RECORD_INDEX_NAME)
      val isExist =
        client.admin().indices().exists(indexExist).actionGet().isExists
      if (!isExist) {
        ...
        val create =
          new CreateIndexRequest(IndexToolsConstants.ALIAS_RECORD_INDEX_NAME).settings(settings)
        client.admin().indices().create(create).actionGet()
      }
