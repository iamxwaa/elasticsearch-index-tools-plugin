...
import com.github.toxrink.indextools.core.config.{AutoCreate, EsSettings}
...
class DefaultJobImpl() extends Job {
  ...
  override def execute(context: JobExecutionContext): Unit = {
    ...
    if (exist.isExists) {
      ...
      builder.putList(ITOOLS_AUTO_CREATE_AVALIABLE_INDEX, list)
      builder.put(ITOOLS_AUTO_CREATE_OPEN, true)
      val auto2 = AutoCreate(new EsSettings(builder.build()))
